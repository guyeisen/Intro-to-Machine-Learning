...
linear = svm.SVC(C=C,kernel='linear').fit(X,y)
poly2 = svm.SVC(C=C, kernel='poly', degree=2).fit(X,y)
poly3 = svm.SVC(C=C,kernel='poly',degree=3).fit(X,y)
models = [linear,poly2 ,poly3]
plot_results(models,["linear","poly2","poly3"],X,y)

poly2_non = svm.SVC(C=C, kernel='poly', degree=2,coef0=1).fit(X,y)
poly3_non = svm.SVC(C=C,kernel='poly',degree=3, coef0=1).fit(X,y)

models = [linear,poly2_non ,poly3_non]
plot_results(models,["linear","poly2_no-homo","poly3_no-homo"],X,y)

for i in range(len(y)):
    if y[i] == 1 and random.random()<0.1:
        y[i] = -1

rbf = svm.SVC(C=C, kernel='rbf', gamma=100).fit(X,y)
models = [poly2_non ,rbf]
plot_results(models,["poly2_no-homo","rbf-gamma=100"],X,y)
